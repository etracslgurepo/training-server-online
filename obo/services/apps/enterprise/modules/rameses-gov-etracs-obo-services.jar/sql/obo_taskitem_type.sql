[getAllowedTypes]
SELECT DISTINCT y.objid, y.title, y.sortindex, y.appclassid, y.processname
FROM 
(SELECT z.* FROM 
(SELECT bet.objid, bet.sortindex, bet.title, swf.name, bet.appclassid, bet.processname, 
...
INNER JOIN sys_wf_node swf ON swf.processname = bet.processname 	
...
SELECT
	...
	INNER JOIN sys_wf_node sn ON sn.processname = typ.processname AND sn.name = bt.state 
