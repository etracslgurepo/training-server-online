...
FROM
   (SELECT 
      ...
      ( SELECT dtcreated 
        ...
        ORDER BY _tit.dtcreated DESC LIMIT 1  ) AS startdate,

      ( SELECT dtcreated FROM obo_app_taskitem_task WHERE state = 'end' 
        AND refid = a.objid ORDER BY dtcreated DESC LIMIT 1 ) AS enddate
...
SELECT 
   ...
   $P{appid} AS appid,
   ti.typeid,
   IFNULL(ti.revision,0) + 1 AS revision,
