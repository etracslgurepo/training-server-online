...
FROM (
	...
	(SELECT TOP 1 dtcreated FROM obo_app_taskitem_task 
	   WHERE refid = ti.objid ORDER BY dtcreated ASC ) AS startdate,
	(SELECT TOP 1 dtcreated FROM obo_app_taskitem_task 
	   WHERE refid = ti.objid AND state = 'end' ) AS enddate 
