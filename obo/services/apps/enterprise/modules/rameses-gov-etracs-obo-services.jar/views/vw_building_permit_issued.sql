DROP VIEW IF EXISTS vw_building_permit_issued;
CREATE VIEW vw_building_permit_issued AS
SELECT 
   bp.*,
...
FROM obo_doctype dt 
INNER JOIN obo_app_doc ac ON ac.doctypeid = dt.objid AND dt.code = 'BUILDING_PERMIT'
INNER JOIN obo_control oc ON ac.controlid = oc.objid 
INNER JOIN building_permit bp ON ac.appid = bp.objid 
INNER JOIN obo_app app ON bp.objid = app.objid  
