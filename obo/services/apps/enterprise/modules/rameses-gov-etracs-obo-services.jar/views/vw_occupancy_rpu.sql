...
INNER JOIN obo_app_doc bdc ON bdc.appid = bp.objid 
INNER JOIN obo_doctype bdt ON bdc.doctypeid = bdt.objid AND bdt.code = 'BUILDING_PERMIT' 
INNER JOIN obo_control bc ON bc.docid = bdc.objid 

INNER JOIN obo_app_doc odc ON odc.appid = oc.objid 
INNER JOIN obo_doctype odt ON odc.doctypeid = odt.objid AND odt.code = 'OCCUPANCY_CERTIFICATE' 
INNER JOIN obo_control occ ON occ.docid = odc.objid 
