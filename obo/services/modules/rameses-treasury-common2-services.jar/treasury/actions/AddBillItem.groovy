...
class AddBillItem implements RuleActionHandler  {
	...
	public void execute(def params, def drools) {
		...
		if(params.account) {
			...
			if(params.billcode instanceof KeyValue ) {
		...
		def d = facts.find{ it.hashCode() == billitem.hashCode() };
		if( !d ) {
			facts << billitem;
		}
		else if( mergetype ) {
			if( mergetype == "ADD") {
				d.amount = d.amount + billitem.amount;
				d.amtpaid = d.amtpaid + billitem.amtpaid;
			}
			else {
				d.amount = billitem.amount;	
				d.amtpaid = billitem.amtpaid;
			}
		}
