...
select 
	b.bin, b.tradename, b.businessname, b.address_text as businessaddress, 
	t2.*, (t2.q1 + t2.q2 + t2.q3 + t2.q4) as totalpaidqtr 
from ( 

	select 
		t1.businessid, sum(t1.amtdue) as amtdue, sum(t1.amtpaid) as amtpaid, 
		sum(t1.surchargepaid) as surchargepaid, sum(t1.interestpaid) as interestpaid, 
		(sum(t1.amtdue) - sum(t1.amtpaid)) as balance, 
		sum(t1.q1) as q1, sum(t1.q2) as q2, sum(t1.q3) as q3, sum(t1.q4) as q4 
	from ( 

		select 
			b.objid as businessid, sum(r.amount) as amtdue, sum(r.amtpaid) as amtpaid, 
			0.0 as surchargepaid, 0.0 as interestpaid, 0.0 as balance, 
			0.0 as q1, 0.0 as q2, 0.0 as q3, 0.0 as q4 
		from ( 
			select distinct p.applicationid 
			from business_payment p 
			where p.refdate >= $P{startdate} 
				and p.refdate <  $P{enddate} 
				and p.voided = 0 
		)t0 
			inner join business_application a on a.objid = t0.applicationid 
			inner join business b on (b.objid = a.business_objid and b.permittype = $P{permittypeid})
		...
		select 
			b.objid as businessid, 0.0 as amtdue, 0.0 as amtpaid, 
			0.0 as surchargepaid, 0.0 as interestpaid, 0.0 as balance, 	
			sum(case when month(p.refdate) between 1 and 3 then p.amount end) as q1, 
			sum(case when month(p.refdate) between 4 and 6 then p.amount end) as q2, 
			sum(case when month(p.refdate) between 7 and 9 then p.amount end) as q3, 
			sum(case when month(p.refdate) between 10 and 12 then p.amount end) as q4 
		from business_payment p 
